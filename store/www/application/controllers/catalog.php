     ...
     function index()
	 {
		 ...
		 foreach($Items as $data){
			...
			if(key($data)=="pagination"){
				...
				if(is_numeric($page))
					$pages.='<a class="'.$info[2].'" href="/catalog?p='.($page+1).'" style="font-size:16px; font-family:HelveticaLight">'.($page+1).'     </a>';
		
				}
				
				$pages='<div class="pagination" style="margin:45px 15%; font-size:18px; font-family:HelveticaLight; clear:both">Страница '.$pagination['activ_page'].' из '.(count($pagination)-1).': '.$pages.'</div>';
				$this->pager=$pages;
			}		 
	 }
